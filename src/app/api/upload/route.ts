...
export async function POST(request: NextRequest) {
  try {
    ...
    // Accept field name 'file' (primary) or 'image' (fallback for some legacy callers)
    ...
    const safeExt = ALLOWED_EXTS.includes(ext) ? ext : (file.type.split('/')[1] || 'jpg');
    ...
    if (admin) {
      await logAdminAction(
        admin.id,
        'UPLOAD_FILE',
        `Uploaded ${filename} (${file.name}, ${file.size} bytes)`,
        getClientIp(request)
      ).catch(() => {});
    }
