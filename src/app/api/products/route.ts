...
import { validateProductPurchase } from '@/lib/tier-system';
  ...
  try {
    ...
    if (!productCheck.ok) {
      return NextResponse.json(
        { success: false, error: productCheck.error },
        { status: 400 }
      );
    }
    ...
    // ★ Defensive backstop: PER-ASSET-UNIQUE-RULE block di atas sudah pastikan
    // user belum punya active investment untuk aset yang sama (same tier index).
    // Cek ini tinggal backstop kalau ada data legacy yang inconsistent (Purchase
    // active untuk produk yang sama tapi Investment-nya hilang). Praktis tidak
    // akan ke-trigger di VPS sehat.
    ...
    // ★ v19 PER-ASSET-UNIQUE-RULE: user boleh punya banyak aset aktif (beda tier).
    //   YANG DILARANG: dobel aset yang sama (same tier index). Block beli di atas
    //   sudah handle (return error kalau aset yg sama masih active).
    ...
    const purchase = await db.$transaction(async (tx) => {
      ...
      // as completed — block beli di atas sudah pastikan user belum punya aset
      // yang sama (same tier index). Other-tier assets remain untouched.
      // Cron akan credit SEMUA active investments jam 00:00 WIB.
