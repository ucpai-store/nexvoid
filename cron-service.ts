...
console.log(`[Cron Service v2.7] 🚀 Running on port ${PORT} — marker: PROFIT-CLEANUP-V3.0-20250630`);
...
cleanupDuplicateProfits().then((report) => {
  console.log(`[Cron Service] 🧹 v3.0 Profit Cleanup done: removed ${report.duplicateEntriesRemoved} duplicate entries, recalculated ${report.investmentsRecalculated} investments, corrected ${report.usersBalanceCorrected} users (total ${report.totalBalanceCorrected} over-credit removed)`);
}).catch((e) => {
  console.error('[Cron Service] v3.0 Profit Cleanup failed (non-fatal):', e.message);
}).finally(() => {
