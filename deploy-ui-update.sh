...
echo ""
echo "Changes live:"
echo "  ✓ PAKET (halaman Paket): TANPA gambar/banner — pakai VIP tier badges"
echo "    (Starter/Silver/Gold/Platinum/Diamond, model InvestmentPackage)"
echo "  ✓ PRODUK (halaman Produk): DENGAN gambar/banner sesuai yg di-set admin"
echo "    (model Product dengan banner emas, sama seperti #admin-products)"
echo "  ✓ Sistem pembelian diubah ke aturan no-duplicates + 1-active-only:"
echo "      • Setiap produk/paket hanya bisa dibeli SEKALI"
echo "      • Boleh beli mana saja yg belum dimiliki (TIDAK harus berurutan)"
echo "      • 1 produk/paket aktif saja — beli baru menggantikan aktif lama"
echo "  ✓ PROFIT wajib masuk SALDO UTAMA (mainBalance) jam 00:00 WIB setiap hari kerja"
echo "  ✓ PROFIT SKIP di weekend (Sabtu & Minggu) — libur, semua aktivitas mati"
echo "  ✓ Cron-service di-restart (sebelumnya mati → profit belum masuk, sekarang fixed)"
...
echo ""
echo "⚠️  PENTING: Pastikan cron-service (port 3032) running di VPS:"
echo "    cd /home/nexvo/nexvo/mini-services/cron-service && nohup bun run index.ts > cron.log 2>&1 &"
echo "    (atau restart via PM2 jika sudah dikonfigurasi)"
echo ""
